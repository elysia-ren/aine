...
> **配套文档 2 / 3**
>
> 本计划把《Aine 语言完整设计方案定稿版》（V5.8）的 **Phase 0-7 技术路线** 与《Aine 1.0 最终交付物清单》的 **22 项交付物** 连接成一份可执行计划：
>
> - 交付物 × Phase 映射；
...
|---|---|---|---|---|
| 1 | aine 编译器 | Phase 0-4 主线，Phase 6 codegen | MVP + 1.0 | 最大技术风险，见 §5 R1 |
...
| 22 | Conformance | Phase 1 起持续 | MVP（核心子集）+ 1.0 | MVP：类型/Ownership/Value Aine/Summary |
...
| 交付物 | MVP 范围 |
|---|---|
| aine | 完整流水线（Lexer→…→Codegen）桌面 x64（Windows 优先）；Ownership/Value Aine/Summary 单模块 + 跨模块；增量 + 缓存；诊断 + Quick Fix 信息；§71.1/71.2/71.4 通过 |
...
| flowpkg | 最小：依赖解析、aine.toml/flow.lock、本地缓存、离线构建 |
| 测试框架（23） | 单元测试 + 最小 UI 测试（事件驱动断言） |
| 文档 | The Aine Book 第 1-15 章（到 SQLite/HTTP）、Language Reference 初版、UI Guide 初版 |
| Build/Publish | single / portable（Windows）；签名预留接口 |
| Conformance | 类型 / Ownership / Value Aine / Summary / 诊断 核心子集 |
...
| # | 交付物 | 完成定义（DoD） | 验收方式 |
|---|---|---|---|
| 1 | aine | 全流水线可用；§71.1-71.7 全部通过；编译时间原则（§75）满足；D6 精度不变量成立；增量编译与缓存失效正确 | Conformance + 基准测试（10/100 MB/1 GB，§74） |
...
| 7 | Profiler | CPU/Memory/Allocation/I-O/Task/UI 六区 + Aine 专属（Materialization/Large Copy/Borrow 结果） | 对基准应用产出可解释报告 |
| 8 | flowpkg/Registry | aine.toml/lock/workspace/feature/target/cache/offline；Registry 可发布/获取包 | 发布-拉取-离线构建闭环测试 |
...
| 11 | The Aine Book | §11 22 章全；前半（1-10 章）不出现生命周期/Borrow Checker 术语（§11 原则） | 无 Rust 经验测试者完成 §73 清单 |
...
| 里程碑 | 内容 | 退出标准 |
|---|---|---|
| **M0 设计收口** | 《Aine 设计决策登记簿》D1-D11、D13 全部评审确认 | 登记簿全绿；正式文档同步 |
...
| **M8 五平台与 1.0**（Phase 6/7） | 桌面三平台 + 移动端桥；Build/Publish 全模式；Registry；文档全套；Conformance 全绿 | 22+1 项 DoD 全绿 → Aine 1.0 完整 |
...
Codegen 采用 LLVM（D10）；aine 依赖 LLVM 工具链，需在 M1 前确定版本锁定与分发策略。
...
```text
《Aine 语言完整设计方案定稿版》(V5.8)   ← 语义层：是什么、怎么想（冻结）
《Aine 1.0 最终交付物清单》             ← 产品层：交付什么（最终愿望清单）
《Aine 设计决策登记簿》(本文档配套 1)    ← 决策层：实现前必须确定的定义
《Aine 1.0 执行计划与完成定义》(本文件)  ← 执行层：何时、以何顺序、如何验收
```
...
> Aine 1.0 的完成定义不是"22 项清单全部做出"，而是 **"M6 的 MVP 通过 §72 联合验收并发布" + "M8 时 22+1 项 DoD 全绿"**。所有执行决策以 M6 为第一目标，R1 为第一风险，D1-D11、D13 为第一优先的待决事项。
