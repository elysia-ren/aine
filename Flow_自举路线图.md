# Flow 自举路线图（Self-Hosting Roadmap）

> **版本**：B1.0（2026-02-14）
>
> 目标：**最终用 Flow 语言实现 Flow 编译器本身**；中途尽量用 Flow 开发，
> 在过程中修复错误、优化语法。
>
> 当前基础：flowc（Rust）已具备完整前端（词法→语法→HIR→解析→类型→值流）
> + 树遍历解释器（`flowc run` 可执行 Flow 程序，134 测试）。
...
```text
A. 参考实现（现在）        Rust flowc：前端 + 分析 + 解释器
        │
B. Flow 表达力验证（本轮起）  用 Flow 写纯逻辑组件（lexer/fmt/json/...），
        │                  由 Rust 解释器执行 → 暴露语法缺口 → 优化语法
        │
C. Flow 写编译器前端         用 Flow 写 lexer/parser/HIR/类型/分析，
        │                  输出 Flow 可处理的数据（结构体/列表），由解释器运行
        │
D. Flow 写后端              用 Flow 写 codegen：
        │                  ① Flow → C 转译器（自举经典路径，C 编译器作后端）
        │                  ② 最终：Flow → LLVM IR（设计稿 D10）
        │
E. 自举闭环                 Flow 编译器（用 Flow 写）转译自身为 C →
        │                  C 编译器产出原生编译器 → 原生编译器编译自身 →
        │                  Rust flowc 退化为引导工具
        │
F. 最终形态                 完整 Flow 编译器 100% Flow 实现，编译为原生，
                            能编译自身；语法在自举过程中持续打磨
```

## 2. 中途用 Flow 开发的内容（按价值排序）

| 顺序 | 组件 | 用途 | 暴露的语法需求 |
|---|---|---|---|
| 1 | lexer.flow | 词法分析器 | 字符串/字符操作、结构体列表、错误传播 |
| 2 | json.flow | JSON 解析器 | 递归、Option/Result 传播、Map 需求 |
| 3 | fmt.flow | 格式化器 | AST 遍历、递归、大型结构体 |
| 4 | symbols.flow | 符号表 | **Map/Dict 类型**（编译器刚需） |
| 5 | parser.flow | 语法分析 | **enum 变体 + match 模式**、递归下降 | B4-M1 表达式级已落地（parser_expr.flow） |
...
- **每个 Flow 组件必须能 `flowc run` 运行并有断言测试**——先证明能跑，再谈性能。
- **语法缺口先记入本清单，再最小化修改**（Rust 解释器/前端 + Flow 语法规范 G1.0 同步更新）。
- **Rust 实现是真相来源（参考编译器）**：Flow 组件行为必须与 Rust 参考一致（差分测试）。
- 最终形态前，**Rust flowc 保留**：即使自举完成，也保留参考实现用于测试与回退。

## 5. 关键风险

- **性能**：解释器执行 Flow 编译器会很慢（lexer 自举时可为原始速度）。缓解：先保证正确，性能优化（字节码 VM/原生）后置。
- **语言表达力**：Flow 必须能表达指针/数组/字典等编译器数据结构。缓解：Map 类型 P1 立项。
- **范围蔓延**：自举期间不做 UI/并发 runtime（设计稿 Phase 3+ 保持 Rust/后续阶段）。

## 6. 当前里程碑

- [x] B0：Rust 参考实现（前端+分析+解释器，134 测试）
- [x] **B1：lexer.flow 用 Flow 写出并运行**（140 测试）—— Flow 成功表达词法分析器；
  暴露并修复 5 个实现缺陷（见下）
- [ ] B2：json.flow / fmt.flow
...
- [x] **B2：json.flow（Flow 写 JSON 解析器，155 测试）**：
  对象/数组/字符串（转义）/数字/布尔/null 递归下降解析 + 序列化 + 往返验证；
  写 Flow 过程中修复 8 个实现缺陷（? 传播、match 语句臂、Option/Result 模式、
  字符串拼接类型、发散臂、单元变体值/模式、None 值）——详见日志
- [x] **B4-M1：parser_expr.flow（表达式 Pratt 解析器，157 测试）**
- [x] **B4-M2：parser_stmt.flow（语句级解析器，159 测试）**：
  let(mut/类型注解)/if-else/while/for..in/return/赋值/块 + fn 定义，
  4 组自断言全过；暴露并修复表达式块内 return 不传播（RtError::Return）；
  Flow 无元组解构 → TyResult 结构体；新增 += 等复合赋值
- [x] **B4-M3：parser_stmt.flow 全量 items（162 测试）**：
  struct/enum/impl/mod + match 语句 + 空元组 + 泛型类型解析；
  修复解释器栈溢出（Rc<FnDef> + 512MB 栈线程）
- [x] **B4-M4：parser_stmt.flow 解析真实 Flow 程序（自举验证）**：
  注释/f-string/结构体字面量/?/for-in 集合迭代；
  用例 9 解析 lexer.flow 源码；修复一元运算符 trailing 消费 bug
- [x] **B4-M5：parser_stmt.flow 终极自解析（11 用例）**：
  Flow 解析器解析自身类型定义与 parse_let 函数；
  补齐闭包/mut 形参/索引切片解析
- [x] **B5-M1：transpiler.flow（Flow 写 Flow→C 转译器）**：
...
- [x] **B5-M6：完整闭环验证**：
  Flow→C→zig cc 编译→运行输出 55（与解释器一致）；修复 match 块值语义
- [x] **B5-M7：字符串拼接（fl_strcat）+ 函数返回类型表**：
  hello Flow 闭环验证；print 格式 fn 感知（%d/%s）
...
- [x] **B5-M17：内置 Option/Result 类型映射（fl_opt）**：
  ...
  用例 11-12；验证：transpiler 12 用例全过 + flowc 159+3 全绿
- [x] **B5-M18：transpiler 自转译闭环（fixpoint）**：
  self_host.exe（Flow 生成 C 编译）13 用例全过 + 自生成输出与 flowc 逐行全同
...
- [x] **B5-M19：通用编译验证（端到端自举闭环）**：
  ...
  case14/15 编译真实 Flow 程序（fib.flow / hello.flow：递归、for 区间、
  中文串、if-else、尾返回、f-string 变量+调用插值）→ zig cc → 原生运行
  输出与 flowc run 逐行一致（diff 0）；fixpoint 保持 diff 0。
- [x] **B5-M20：字符串方法 split/trim/replace + 字面量后缀解析 + 方法调用类型推断**：
  ...
  类型表接入 var_info/c_stmt/c_print_stmt；case16 strings.flow 端到端
...
- [x] **B5-M21：数组字面量换行分隔 + Err 载荷绑定 + Vec<struct> 类型推断**：
  ...
  类型>（空数组保持 infer 保 push 回退）；case17 records.flow（Vec<struct>
...
- [x] **B5-M22：方法链 iter/map/sum（闭包内联）**：
  ...
  （4.0 而非 4）；case18 mapsum + case19 todo.flow（方法链+Result+? 传播）
  端到端 diff 0；fixpoint diff 0。剩余缺口：已全部闭环（闭包独立值、
  import/module 均已实现，G2.0-③ 后为 module 形态）。
- [x] **B5-M23：Map 类型 C 后端 + json.flow 端到端编译**：
  ...
  插值 fl_bool；case20 json.flow（Map<String,Json> + 枚举 Map 载荷）端到端
...
- [x] **B5-M24：char→String 实参自动转换（函数形参类型表）**：
  ...
  json.flow 的 is_digit 恢复 String 仍端到端 diff 0；case21 char-arg；
...
- [x] **B5-M25：嵌套泛型类型（Map<String, Vec<T>>）+ case13 稳定性定论**：
  ...
  values）端到端 diff 0；fixpoint 9118 行全同（25 用例）；flowc 159+3
...
- [x] **B5-M26：import / module（文件模块 + 扁平注册）+ 上帝文件首次拆分**：
  ...
  Flow 转译器 SMod/SImport 解析 + flatten_stmts 打平 + c_program 薄壳重算
  类型表）；`import`/`use` 为声明标记（引用已有名字或占位）；case26
  modtest（modtest_main + modtest_util）解释器/原生 diff=0；狗粮拆分
  fltype.flow（first/second_type_arg、method_ret、payload_member 四工具），
  Rust 加载与 Flow 自转译双路径验证。工程期决策：MVP 后按 §71.2 再演进
  命名空间。剩余缺口：f-string 方法链插值、闭包独立值。
- [x] **B5-M26.1：transpiler.flow 上帝文件拆分完成（六模块）**：
...
- [x] **M6-STD-1：标准库启动（M6 首个交付物破土）**：
  flowc/stdlib/{math,strutil,collections}.flow（纯 Flow）；mod 搜索路径
  同目录 → stdlib/（Rust 与 Flow 两侧加载器）；stdtest.flow 端到端 diff=0；
...
- [x] **B5-M28.1：发射层修复 + 值流子集激活**：
  ...
  fl_strcat_own/fl_strdup_lit 双实现（Flow 语义 + C 内建）；27 用例 +
...
- [x] **B5-M32：自举线定稿**——match 尾构造:rewrite_lift_calls 采用语句式
  ...
  **自举线 100% 基线达成**:语言特性层全部自举(嵌套类型/模块/stdlib/独立
  与捕获闭包/值语义/值流子集),六模块+三趟全 Flow,fixpoint 11110 行全同。
- [x] **里程碑 A：Flow 工具链自给**——
  ...
  1.4 pkg_tool（flow.toml 极简解析 + myapp 脚手架 + build 产物校验）。
...
- [ ] M6-FMT-1：fmt_tool.flow——Flow 原生格式化工具（幂等 + 可重解析）
- [ ] M6-FMT-2：fmt_tool 输出对齐 flowc fmt（diff=0）
- [ ] M6-TCK：typeck 诊断分期迁移 Flow（前置：Index 节点携带 span）
- [x] M6-TEST-1：测试框架（M23）—— `@test` 属性 + `flowc test` 命令
  （源码顺序发现、每测独立解释器+大栈线程隔离、Err/运行时错误/崩溃三类失败判定、
  汇总报告+退出码）；验收集 examples/testdemo.flow（5/5 过）与
  examples/testfail.flow（失败演示）；回归测试 3 项入 cargo test（162 全绿）
- [x] M2：Conformance 子集 —— 正向行为集 conformance/（类型 10/值语义 6/
  闭包 6/错误处理 5/模块 4 = 31 用例，flowc test 运行全过）；
...
- [x] B5：Flow → C 转译器（自举闭环起点）— 前端组件（lexer/json/parser/transpiler）
  已全部由 Flow 实现并自举替换 Rust 对应组件

## 7. B1 暴露并修复的缺陷（写 Flow 的价值证明）
...
4. **解释器：Vec 方法不可变语义**（push 返回新 Vec 被丢弃）→ 变量接收者自动重绑定（Flow 可变用法）
